...
class Gan():
    ...
    ["binary_cross_entropy", "categorical_cross_entropy" ], [ "binary_cross_entropy", "L1" ], [ "binary_cross_entropy", "L2" ], ["binary_cross_entropy", "perceptual_wasserstein"], ["binary_cross_entropy", "categorical_cross_entropy"],
    [ "wasserstein", "L1" ], [ "wasserstein", "L2"], [ "wasserstein", "perceptual_wasserstein" ] , ["wasserstein", "categorical_cross_entropy"],
    [ "L2" , "L1" ], ["L2" , "categorical_cross_entropy"] , ["L2", "perceptual_wasserstein"], 
    [ "L1", "categorical_cross_entropy"] , ["L1", "perceptual_wasserstein"],
    ["perceptual_wasserstein","categorical_cross_entropy" ] ], 
        "cas": [0,1], 
        "denominator" : denominator, # à remplacer plus tard par une liste pour le dénominateur dans le main 
        "total_epoch" : 100,
        "epoch" : 0,
        "nom_chemain" : [],
        "y_true" : "true",
    ...
    def sampling(self, mu_log_variance):
        ...
        random_sample = mu + keras.activations.exponential(log_variance/2) * epsilon
    ...
    def build_encoder(self):
        ...
        latent_repr = Lambda(self.sampling, name="encoder_output")([mu, log_var])
    ...
    def compile(self):
        ...
        loss = self.loss_
        self.model_disc.compile(loss = loss, optimizer=optimizer, metrics=['accuracy'])
    ...
    def recuperer_images(self, epoch , name_loss, num_cas, d_loss , denominator, path, imgs):
        ...
        headers = ["original","generated" ,"num_epoch", "name_loss", "loss", "list_cas", 'numcas', "denominator",  "PSNR score","SSIM score"]
        ...
        for j in range(c):
            ...
            if self.grey == True:
                print("There")
                I = imaged[:, :, 0]
            name = path + "_"+ str(i) +"_" + str(j) +"_"+ str(epoch) +"_" + str(name_loss) +"_" + str(denominator) +".png"
            ...
            for i in range(r):
                ...
                self.write_csv("bilan.csv" , [originale , imaged, epoch, name_loss, d_loss, self.data["list_cas"], self.data["num_cas"], denominator, scores[0],scores[1]])
...
############################ Training #################################################
    def train(self, epochs, batch_size=10, sample_interval=50):
        ...
        X_train = X_train.reshape(X_train.shape[0], X_train.shape[1], X_train.shape[2], self.channels)
        ...
        for epoch in range(epochs):
            ...
            imgs = X_train[idx]
            ("SHAPE ===============", imgs.shape)
            latent_fake = self.encod.predict(imgs) # np.expand_dims(np.squeeze(imgs), axis=3))# erreur de dimension sinon 10, 250, 250, 1, 1 alors qu'onveut n= 4
            ...
            new_loss_list = [epoch, d_loss[0], 100*d_loss[1], g_loss[0], g_loss[1]]
            ...
            if epoch % sample_interval == 0:
                ...
                self.recuperer_images(epoch, name_loss, self.data["num_cas"], d_loss[0], self.data["denominator"], "C:/Users/Clara/Scripts/Segmentation/AAEGAN_Oumaima/projet_oumaima/aae-gan/dataset_out/", imgs)
            # mean_ssim, list_ssim, list_subname, median_ssim, sliding_mean, grouped, group, percent, filename_ssim = self.sample_images(epoch, imgs)
            # self.sample_images(epoch, imgs)

            # self.recuperer_images(epoch , d_loss[0] , self.data[4], "/dataset_out/", imgs)
            
            #stop counting time
            finish_time = time.time() - start_time
            print("--- %s seconds ---" % (finish_time))
            self.write_csv("temps_execution.csv" ,  [epoch, name_loss, self.data["num_cas"], finish_time] )
...
if __name__  == "__main__":
    ...
    # class_.renameimage("C:/Users/ASUS/Desktop/awesomeprojects/aae-gan/dataset_resize/")
    class_.renameimage("C:/Users/Clara/Scripts/Segmentation/AAEGAN_Oumaima/projet_oumaima/aae-gan/dataset_resize/")
